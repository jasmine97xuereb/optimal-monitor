...
let rec random_formula (size:int) (max_var:int) (actions:action list): Ast.formula =
  ...
  if size = 0 then raise Random_formula_exception
    ...
    match Random.bool() with
      | true  -> Min("X" ^ variable(max_var), random_formula (size-1) (max_var+1) actions)
      | false -> Max("X" ^ variable(max_var), random_formula (size-1) (max_var+1) actions)
    ...
    match Random.int 3 with
      ...
      | 1 -> Min("X" ^ variable(max_var), random_formula (size-1) (max_var+1) actions)
      | 2 -> Max("X" ^ variable(max_var), random_formula (size-1) (max_var+1) actions)
      | _ -> raise Random_int_exception
  else
